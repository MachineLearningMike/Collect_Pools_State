...
def fetch_pool_events(chain):
    ...
    try:
        pandas_gbq.to_gbq(df, f'{project_id}.Time_block.tbl_{chain}_time_block', project_id=project_id, credentials=credentials, if_exists='append', table_schema=tbl_schema)
    except:
        print('error to_gbq')
...
def main(req1, req2):
    ...
    #         exec(f"{chain}_thread = threading.Thread(target=delete_table, args =('nice-proposal-338510', 'nice-proposal-338510.STG_related.tbl_{chain}_time_block'))")
