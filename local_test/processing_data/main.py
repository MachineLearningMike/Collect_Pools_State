...
def process_data():
    ...
    pandas_gbq.to_gbq(df, tbl_stablecoin_id, project_id=project_id, credentials=credentials, if_exists='replace', table_schema=tbl_schema)
    ...
    last_lowest = min(last_list.values())
    df = pandas.DataFrame(all_fields_dataset)
    df.drop(df[df['TS_FROM'] >= last_lowest].index, inplace=True)
